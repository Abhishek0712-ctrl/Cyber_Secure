...
const Contact = () => {
  ...
  const sendMessage = (e) => {
    e.preventDefault();
    setLoad(true);
    if(!msgParam.name || !msgParam.email || !msgParam.subject || !msgParam.phoneNo || !msgParam.queryType || !msgParam.message){
      setLoad(false);
      setAlert({
        show: true,
        type: false,
        msg: "Please fill all the mandetory fields",
      });
      return;
    }
    else{
      axios
        .post(`${process.env.REACT_APP_API_URL}/contact`, msgParam)
        .then((res) => {
          setLoad(false);
          setAlert({
            show: true,
  ...
  // };
  const checkError = (param) => {
    if (errorParam && Array.isArray(errorParam)) {
        return errorParam.find((error) => error.param === param) !== undefined;
    } else {
        return false; // Handle null or undefined errorParam
    }
};
  return (
    <>
    ...
    >
      ...
      >
        <Stack sx={{ color: "whitesmoke" }} spacing={1}>
          ...
          >
            <strong>Cyber Security Awarness</strong>
          </Typography>
          <Typography variant="body2">
            Fill up the form and our Team will get back to you.
          </Typography>
          <Typography variant="body2">
            You can also drop us a mail.
          </Typography>
          <Stack spacing={3} sx={{ padding: "25px 0px" }}>
            <Button
              ...
              color="warning"
              onClick={() =>
                window.open("mailto:xyz@gmail.com", "_blank")
              }
            >
        ...
        >
          ...
          <FormControl fullWidth>
            ...
            >
              ...
              <MenuItem value={"ORDER_HELP"}></MenuItem>
